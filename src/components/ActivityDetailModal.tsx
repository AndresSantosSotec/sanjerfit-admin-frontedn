      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm">
              <strong>Estado:</strong> {activity.status ?? 'pendiente'}
            </p>
          </div>
        ...
        <DialogFooter className="gap-2">
          <Button
            variant="destructive"
            onClick={() => onValidate(activity.id, false)}
            disabled={saving || activity.status === 'rechazada'}
          >
            Rechazar
          </Button>
          <Button
            onClick={() => onValidate(activity.id, true)}
            disabled={saving || activity.status === 'aprobada'}
          >
            Validar
          </Button>
        </DialogFooter>
